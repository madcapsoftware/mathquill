...
CSS_SOURCES =  \
  $(CSS_DIR) \
  $(CSS_DIR)/mixins/css3.less \
  $(CSS_DIR)/mixins/display.less \
  $(CSS_DIR)/mixins/fonts.less \
  $(CSS_DIR)/editable.less \
  $(CSS_DIR)/font.less \
	$(CSS_DIR)/math.less \
  $(CSS_DIR)/matrixed.less \
  $(CSS_DIR)/selections.less \
  $(CSS_DIR)/textarea.less
...
CLEAN += $(BUILD_DIR)/*
PRODUCT_DIR = ./../Resources/ClientApp/

DISTDIR = ./mathquill-$(VERSION)
DIST = $(DISTDIR).tgz
CLEAN += $(DIST)
#CLEAN += ./../Resources/ClientApp/$(DISTDIR)
